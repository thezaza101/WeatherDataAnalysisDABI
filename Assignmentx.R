...
## Create the monthly data - used in section 3 of the assignment
monthlyData <- data %>% 
  mutate(Month = month(as.Date(Date,format="%d/%m/%Y"))) %>%
  group_by(Month) %>%
  mutate(RainToday=sum(Rainfall.mm.>0))
         
x <- aggregate(monthlyData[, 2:23], list(monthlyData$Month), mean)

write_csv(x,"Data\\MonthlyStat.csv")
...
rm(file,files_list,temp_data, frmtdate, rawData, compassDir, compassData,seasons,col_idxs,col_idx,dataANN, monthlyData, x)





## Find the Numeric variables that are closely correlated
